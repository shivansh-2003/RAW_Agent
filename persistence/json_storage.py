...
import logging
import threading
from typing import Dict, List, Optional, Any, Set, Union, Tuple, Type
...
from persistence.db_interface import (
    AgentDatabase, RulesDatabase, AuditTrailDatabase, NeuralSignatureDatabase
)
...
logger = logging.getLogger("shadow_persistence.json_storage")


class JsonStorage:
    """
    Base JSON file storage for the persistence layer
    Provides common functionality for JSON-based storage implementations
    """
    
    def __init__(self, file_path: str, auto_save: bool = True, lock: Optional[threading.RLock] = None):
        """
        ...
        Args:
            ...
            lock: Optional thread lock for thread safety
        """
    ...
    def initialize(self) -> bool:
        """Initialize the storage, loading data from file if it exists"""
        with self.lock:
            try:
                ...
                if os.path.exists(self.file_path):
                    ...
                    # Create empty data structure and save it
                    self.data = {"items": {}, "metadata": {"created_at": datetime.datetime.utcnow().isoformat()}}
                    self.save()
                
                self.initialized = True
                return True
            except Exception as e:
                logger.error(f"Failed to initialize JSON storage at {self.file_path}: {e}")
                return False
    ...
    def save(self) -> bool:
        ...
        with self.lock:
            try:
                ...
                self.data["metadata"]["last_updated"] = datetime.datetime.utcnow().isoformat()
                
                # Write to temporary file and rename to avoid corruption
                ...
                logger.error(f"Failed to save JSON storage to {self.file_path}: {e}")
                return False
    
    def close(self) -> None:
        """Close the storage, saving if needed"""
        with self.lock:
            if self.initialized and self.auto_save:
                self.save()
            self.initialized = False
